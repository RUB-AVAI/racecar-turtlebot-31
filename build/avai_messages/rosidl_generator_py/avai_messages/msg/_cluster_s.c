...
bool avai_messages__msg__cluster__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("avai_messages.msg._cluster.Cluster", full_classname_dest, 34) == 0);
  }
  avai_messages__msg__Cluster * ros_message = _ros_message;
  {  // x_positions
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->x_positions.data;
  ...
  {  // y_positions
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->y_positions.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
  ...
  {  // angles
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      int16_t * dest = ros_message->angles.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
PyObject * avai_messages__msg__cluster__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("avai_messages.msg._cluster");
  ...
  {  // x_positions
    ...
    if (ros_message->x_positions.size > 0) {
      ...
      double * src = &(ros_message->x_positions.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->x_positions.size * sizeof(double));
      assert(data != NULL);
  ...
  {  // y_positions
    ...
    if (ros_message->y_positions.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->y_positions.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->y_positions.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
  ...
  {  // angles
    ...
    if (ros_message->angles.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int16_t * src = &(ros_message->angles.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->angles.size * sizeof(int16_t));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
